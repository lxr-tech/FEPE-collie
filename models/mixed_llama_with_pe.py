...
class LlamaLayer(nn.Module):
    ...
    def _forward(self, hidden_states, index_ids, mask_or_seqlen):
        ...
        if not self.training and self.config.use_flash:
            assert hidden_states.ndim == 2, f"hidden_states.shape must be (T, H), but got {hidden_states.shape}"
            cu_seqlens = mask_or_seqlen
        else:
            assert hidden_states.ndim == 3, f"hidden_states.shape must be (B, L, H), but got {hidden_states.shape}"
            attention_mask = mask_or_seqlen
        ...
        if not self.training and self.config.use_flash:
            ...
            query, key = self.self_attn["rotary_emb"](query, key, index_ids, flash=False)            
        ...
        if not self.training and self.config.use_flash:
            ...
            attention_score = torch.matmul(query, key.transpose(2, 3)) / math.sqrt(self.head_dim)
...
class LlamaForCausalLM(CollieModelForCausalLM):
    ...
    def forward(self, 
        ...
        if not self.training and self.collie_config.use_flash:
            ...
            inputs = {"hidden_states": self.embed_tokens(input_ids), "index_ids": index_ids, "mask_or_seqlen": cu_seqlens}
        else:
            inputs = {"hidden_states": self.embed_tokens(input_ids), "index_ids": index_ids, "mask_or_seqlen": attention_mask}
