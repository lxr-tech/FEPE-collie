...
for size, a, b, label_dct, file_name in [((17, 15), 3, 3, label_dct0, 'pretrained'),
    ...
    gsp = gridspec.GridSpec(a, b)
    gsp.update(wspace=0.36, hspace=0.22, top=0.96, bottom=0.02, left=0.16 / b, right=1 - 0.16 / b)
    ...
    for col, label in enumerate(list(label_dct)):
        ...
        for row in range(2):
            ...
            ax1 = fig.add_subplot(gsp[row, col])
            color = 'tab:brown'
