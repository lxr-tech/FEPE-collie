...
label_dct2 = {'llama2_7B-qk_100k-hang_500.pkl': ('llama2_7B-books3_100k-500', 'base=500', [4096], 500), 
              'llama2_7B-qk_100k-hang_2608.pkl': ('llama2_7B-books3_100k-2608', 'base=2608', [4096], 2608), }
label_dct3 = {'llama2_7B-qk_100k-hang_40000.pkl': ('llama2_7B-books3_100k-40000', 'base=40000', [4096, 11902], 40000), 
...
label_dct5 = {'llama2_7B-qk_100k-hang_120000_16K.pkl': ('llama2_7B-books3_100k-120000_16k', 'base=120000 16k', [16384, 27336], 120000), 
              'llama2_7B-qk_100k-hang_1000000_16k.pkl': ('llama2_7B-books3_100k-1000000_16k', 'base=1000000 16k', [16384, 102400], 1000000), }

grids0 = {'(a)  Pre-trained LLaMA2 7B': label_dct0, 
          '(b)  Fine-tuned LLaMA2 7B': label_dct1, }
grids1 = {'(a)  RoPE with smaller bases': label_dct2, 
          '(b)  RoPE with larger bases': label_dct3, }
grids2 = {r'(a)  bases smaller than $\beta_0$': label_dct4, 
          r'(b)  bases larger than $\beta_0$': label_dct5, }

for grids, file_name in [(grids0, '10000'),
                         (grids1, 'other'), 
                         (grids2, '16k'), ]:

    fig = plt.figure(figsize=(30, 15), dpi=300)
    gsp = gridspec.GridSpec(3, 5)
    gsp.update(wspace=0.36, hspace=0.22, top=0.94, bottom=0.02, left=0.04, right=0.96)
    
    gsp_s = 0
    
    for grid_title in list(grids):
    
        label_dct = grids[grid_title]
        sub = fig.add_subplot(gsp[:, gsp_s:gsp_s+len(label_dct)])
        ...
        for col, label in enumerate(list(label_dct)):
            ...
            axs = []
            xlim = 16 * 1024 if file_name != '16k' else 32 * 1024
            ylim = (-6.5, 6.5) if grid_title == '(a)  Pre-trained LLaMA2 7B' else ((-3.5, 3.5) if title == 'post-trimming'else (-17.5, 17.5))
            
            for row in range(2):

                ax1 = fig.add_subplot(gsp[row, gsp_s+col])
                color = 'tab:brown'
                ...
                if row == 0:
                    ...
                    ax1.set_title(r'in first %dk' % (xlim // 1024), fontsize=18)
            ...
            con3 = ConnectionPatch(xyA=(0, ylim[0]), coordsA=axs[0].transData, 
                                xyB=(0, ylim[1]), coordsB=axs[1].transData, color='orange', linewidth=2, alpha=0.2)
            sub.add_artist(con3)
            con4 = ConnectionPatch(xyA=(xlim, ylim[0]), coordsA=axs[0].transData, 
                                xyB=(xlim, ylim[1]), coordsB=axs[1].transData, color='orange', linewidth=2, alpha=0.2)
            sub.add_artist(con4)
            # https://medium.com/the-stem/3-minute-guide-to-use-subplots-connection-patch-in-matplotlib-fe50ac0fbeb8
            
            line1=con3.get_path().vertices
            line2=con4.get_path().vertices
            zoomcoords = np.concatenate((line1,line2[::-1]))
            polygen = Polygon(zoomcoords, ec='orange', fc='orange', zorder=100, alpha=0.2)
            sub.add_artist(polygen)
            # https://stackoverflow.com/questions/51268493/drawing-filled-shapes-between-different-axes-in-matplotlib
            
            ax3 = fig.add_subplot(gsp[2, gsp_s+col])
            color = 'tab:brown'
            max_len = 16 * 1024 if file_name != '16k' else 32 * 1024
            ...
            for i, dim in enumerate(d):
                ...
                if title == 'dynamic NTK':
                    ...
                    yd = (np.cos(freq * td) + 2.5 * i) if i+len(d)//2 <= len(d)-1 else np.ones_like(td) * 2.5 * i
                elif title == 'limit index':
                    freq = base ** (-dim / head_dim)
                    td = xd if i+len(d)//2 <= len(d)-1 else np.clip(xd, a_max=4096, a_min=0)
                    yd = np.cos(freq * td) + 2.5 * i
                else:
                    freq = base ** (-dim / head_dim)
                    yd = np.cos(freq * xd) + 2.5 * i
            ...
            ax3.set_title(r'cos of $\theta_n$ for %s' % title, fontsize=18)
        ...
        gsp_s = gsp_s+len(label_dct)
        sub.axis('off')
        sub.set_title(grid_title, fontsize=22, pad=36)

    # plt.tight_layout()
    # plt.subplots_adjust(wspace=0.36, hspace=0.22, top=0.15)
    plt.savefig(f'/mnt/petrelfs/liuxiaoran/projects/FEPE-collie/scripts/blog_scaling_rope-dim-{file_name}.jpg')
